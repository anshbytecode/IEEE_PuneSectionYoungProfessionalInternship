...
export const Signup: React.FC = () => {
  ...
  const onFinish = async (values: any) => {
    if (!isIEEEEmail(values.email)) {
      setErrorMsg('Only @ieee.org and @ieeepune.org emails are authorized to register as administrators.');
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        ...
        message.success('Account created successfully! Welcome to IEEE YP Admin Dashboard!');
    ...
    try {
      ...
      if (data.success) {
        ...
        if (isIEEEEmail(data.admin.email)) {
          ...
          message.success('Successfully registered and logged in with Google!');
          navigate(redirectTo || '/');
        }
      } else {
      ...
      {!isMobile && (
        <div
          style={{
            ...
            padding: '60px 80px',
            background: 'linear-gradient(135deg, #003D5C 0%, #006699 100%)', // Core IEEE brand gradient
            position: 'relative',
            overflow: 'hidden',
        ...
        >
          <motion.div variants={itemVariants} style={{ marginBottom: 32 }}>
            ...
            <Text style={{ color: '#64748B', fontSize: '14px', fontWeight: 500 }}>
              Register below to sign up as a portal administrator.
            </Text>
          </motion.div>
          ...
          <motion.div variants={itemVariants} style={{ textAlign: 'center', marginTop: 12 }}>
            <Text style={{ color: '#64748B', fontWeight: 500 }}>Already have an account? </Text>
            <Link to="/login" style={{ color: '#006699', fontWeight: 700, marginLeft: '4px' }}>
              Sign In
            </Link>
          </motion.div>
        </motion.div>
